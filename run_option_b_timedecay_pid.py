...
class TimeDecayGovernor:
    """
    Governor with aggressive session ramp-down:
    Targets decay as the 15:15 IST cutoff approaches.
    """
    ...
    def get_time_decay_factor(self, current_time) -> float:
        # Intraday minutes from 09:15 to 15:15 = 360 mins
        curr_mins = current_time.hour * 60 + current_time.minute
        cutoff_mins = 15 * 60 + 15
        start_decay = 12 * 60 + 30 # Begin aggressive ramp-down at 12:30 IST
        
        if curr_mins < start_decay:
            return 1.0
        remaining = max(0, cutoff_mins - curr_mins)
        total_decay_window = cutoff_mins - start_decay
        # Linear decay from 1.0 down to a defensive 0.25 floor
        return float(np.clip(remaining / total_decay_window, 0.25, 1.0))
    ...
    def compute_scheduled_targets(self, vol_ratio: float, current_time, risk_inr: float) -> tuple[float, float]:
        ...
        # Base targets
        base_r = 1.25 * np.clip(vol_ratio, 0.9, 1.3)
        base_z = 0.50
        
        # Decay targets dynamically as session ends
...
def run():
    ...
    print("Precomputing 15m features across all instruments...")
    symbol_frames = {}
    ...
    governor = TimeDecayGovernor(friction_floor_inr=85.0)
    active_positions = {}
    closed_trades = []
    armed_symbols = {}
    
    print("\nExecuting Option B with Aggressive Intraday Time-Decay Scheduling...")
    ...
    for t in all_timestamps:
        ...
        for sym, pos in active_positions.items():
            ...
            # Absolute hard boundary at 15:15
            ...
            # Compute Time-Decayed Targets
            r_target, z_target = governor.compute_scheduled_targets(bar['vol_ratio'], bar_time, risk_inr)
            
            # Condition A: Dynamic Decayed R-target hit
            if curr_peak_r >= r_target:
                fill = pos['entry_price'] + (r_target * risk_ticks)
                symbols_to_close.append((sym, fill, "DECAYED_R_TARGET"))
                continue
                
            # Condition B: Decayed Z-target hit (must be at least positive after fees)
            if bar['vwap_zscore'] >= z_target and curr_r >= 0.20:
                symbols_to_close.append((sym, bar_close, "DECAYED_Z_TARGET"))
                continue
        ...
        # 2. EVALUATE ENTRY PERMISSIVE (Strict Cutoff at 13:30 to avoid runway compression)
        ...
        for sym, df in symbol_frames.items():
            ...
            # INHIBIT NEW ENTRIES AFTER 13:30 IST (Guarantees >= 1 hour 45 min runway)
    ...
    print("\n" + "=" * 75)
    print("OPTION B: TIME-DECAY TARGET SCHEDULING RESULTS")
    print("=" * 75)
