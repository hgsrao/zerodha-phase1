...
Repository: /home/shrinivas/ECS_Project_BB09_BB10
Branch: codex/r5-bb09-bb10-laptop
...
| BB09 | `order_timeout_seconds` | 30, [5, 120] | Broker acknowledgement timeout (calibratable half of the effective Gate14 timeout) | ACTIVE_DYNAMIC | registry (`APPROVED_CALIBRATABLE`) | none | `P01DBox.create_order` -> `ProposedOrder.timeout_seconds` (unread downstream); **separately and directly** `orchestrator.py:281-282` `min(order_timeout_seconds_execution, order_timeout_seconds)` feeds `SafetyGateConfig.order_timeout_seconds`, which `Gate14OrderTimeout` (BB07) enforces | calibratable, range 5-120 | Real dual-path consumption: the value changes the constructed order (cosmetically), and — via the `min()` with the fixed `order_timeout_seconds_execution` floor — genuinely tightens (never loosens) Gate14's effective timeout. Proven by `test_order_timeout_seconds_changes_the_constructed_order` and `test_gate14_timeout_is_the_tighter_of_the_calibratable_and_fixed_values`. |
...
| BB10 | `learning_rate_exploration_factor` | 0.05, [0.01, 0.10] | Same `exploration_bias` computation | ACTIVE_DYNAMIC in the registry, but explicitly excluded from the trading search space | none (registry-calibratable) | none | same as above | calibratable at the registry level; excluded from `trading_search_space()` via `revision2/calibration_supervisor.py:CALIBRATION_CONTROL_PARAMS`, with an existing code comment explaining exactly why ("its only current runtime effect is a diagnostic exploration_bias value ... never a trading decision") | Pre-existing, correctly reasoned exclusion; not touched. This is the **only** BB09/BB10 parameter that is registry-calibratable=True. |
...
and frozen identity hash are all **unchanged**). Authority type is
`EXECUTION` for both halves, matching the box identity table above — this
correctly distinguishes BB09/BB10's real execution-mechanics role from
BB03/BB04's `INFORMATION_ONLY` analytics role, per the assignment's
requirement to classify each box honestly rather than defaulting everything
to `INFORMATION_ONLY`.
