"""
Production Trading Engine v1.0 with Critical Fixes

Complete integration of:
- FIX #1: RollingSetpointProvider (unified PID baseline)
- FIX #3: BoundedPIDController (integral anti-windup)
- EntrySignalGenerator (market-aware entry signals)
- Full state machine for order execution, position management, protective stops, exits

Architecture: Order execution state machine with broker reconciliation
- STARTUP → RECONCILING → FLAT (ready for entry)
- ENTRY_PENDING → PARTIAL_POSITION → PROTECTION → PROTECTION_PENDING → MANAGING
- MANAGING → EXIT_CANCEL_SL / EXIT_SUBMIT → EXIT_PENDING → FLAT (completed)

Author: Production Code (Complete Integration)
...
# ==============================================================================
# CRITICAL FIX #3: Bounded PID Controller with Integral Anti-Windup
# ==============================================================================
...
# ==============================================================================
# Entry Signal Generator with Market-Aware Decision Logic
# ==============================================================================

class EntrySignalGenerator:
    """Generates entry signals using rolling baseline + PID confidence."""
...
# ==============================================================================
# Production Trading Engine V1.0
# ==============================================================================

class TradingEngineV1:
    """
    Complete production trading engine with:
    - Order execution state machine
    - Broker reconciliation
    - Position management
    - Protective stops
    - Emergency exits
    - Critical fixes for entry signal generation
    """
    ...
    def __init__(
        ...
        # Entry signal generator with critical fixes
    ...
    def reconcile_startup(self) -> None:
        ...
        try:
            ...
            if self.state.status in {"PROTECTION", "PROTECTION_PENDING", "MANAGING", "EXIT_CANCEL_SL", "EXIT_SUBMIT"}:
                if len(active_pos) != 1 or len(matching_pos) != 1:
                    self.trigger_hard_halt(
    ...
    def step(self) -> str:
        """Main execution loop. Process one poll cycle based on current state."""
        ...
        try:
            ...
            self.trigger_hard_halt(
                f"FAIL_CLOSED_ERROR: Unsupported state '{self.state.status}'."
            )
            return "HALTED"

        except Exception as exc:
            self.trigger_hard_halt(f"Engine exception: {exc}")
            return "HALTED"
