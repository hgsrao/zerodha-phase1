...
def extract_day(symbol: str, day: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for i in range(60,len(d)-HORIZON_BARS-1):
        ...
        base={"timestamp":str(b.timestamp),"symbol":symbol,"bar_index":i,"atr14":scale,"realized_vol_20":float(rv[i]),"return_autocorr_20":float(autocorr[i]),"body_fraction":body,"close_location":close_loc,"gap_atr":gap,"range_atr":rng/scale,"time_of_day":str(b.timestamp)[11:16]}
