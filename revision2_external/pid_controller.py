...
class SimplePIDModelPredictiveControlBox:
    ...
    def build_plan(
        ...
        # The PID adjustment now directly controls the market submission price,
        # not just a confidence multiplier. This makes the controller authority
        # real and measurable.
        #
        # Entry adjustment is already clamped to [-integral_clamp, +integral_clamp]
        # We scale it to basis points (0.01% per unit) so a -0.1 adjustment
        # means "submit 10 bps tighter" (lower for BUY, higher for SELL).
        #
        # This ensures:
        # 1. PID-controlled submission price matches broker's actual fill
        # 2. Negative error (actual_progress > expected) → tighter entry
        # 3. Positive error (actual_progress < expected) → wider/reject entry

        entry_adjustment_bps = entry_adjustment * 100.0  # Scale to basis points

        if side == "BUY":
            # For buys: negative adjustment (tight entry) = lower submission price
            pid_controlled_entry = float(entry_price) * (1.0 - abs(entry_adjustment_bps) * 0.0001)
        else:
            # For sells: negative adjustment (tight entry) = higher submission price
            pid_controlled_entry = float(entry_price) * (1.0 + abs(entry_adjustment_bps) * 0.0001)

        # This is the price we instruct the broker to submit at
        execution_market_price = float(pid_controlled_entry)

        # Broker applies its standard adverse fill on top
        effective_entry = execution_market_price * (
            1.0 + slippage_cost_mult * 0.0005 * (1 if side == "BUY" else -1)
        )

        # Telemetry: track the PID-controlled intent vs actual broker fill
        ...
        # PHASE 2 ADDITIONS: Track PID-controlled entry price authority
        pid_info = {
            "pid_enabled": self.pid_enabled,
            "entry_price_planned": float(entry_price),
            "execution_market_price": float(execution_market_price),
            "pid_intent_bps": pid_intent_bps,  # PID adjustment in basis points
            "slippage_bps": slippage_bps,  # Broker's adverse fill
