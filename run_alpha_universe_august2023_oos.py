...
# Verified Alpha Universe
TARGET_SYMBOLS = [
    "TCS", "HDFCBANK", "BAJFINANCE", "LT", "TATAMOTORS", "INFY"
]

SECTOR_MAP = {
    "HDFCBANK": "BANK", "INFY": "IT", "TCS": "IT",
    "LT": "INFRA", "BAJFINANCE": "FIN", "TATAMOTORS": "AUTO"
}

PORTFOLIO_CAPITAL = 1000000.0
MAX_CONCURRENT_POSITIONS = 3
SLOT_CAPITAL = PORTFOLIO_CAPITAL / MAX_CONCURRENT_POSITIONS  # ₹3,33,333 per slot
...
class TimeDecayGovernor:
    def get_time_decay_factor(self, current_time) -> float:
        ...
        return float(np.clip(remaining / total_window, 0.30, 1.0))
...
def run():
    ...
    print(f"Precomputing August 2023 OOS features for Alpha Universe: {TARGET_SYMBOLS}...")
    symbol_frames = {}
    for sym in TARGET_SYMBOLS:
        ...
        try:
            ...
            # OUT-OF-SAMPLE WINDOW: August 2023
            m = raw[(raw['dt'] >= '2023-08-01') & (raw['dt'] <= '2023-08-31')].copy()
            if len(m) > 300:
    ...
    print(f"Loaded {len(symbol_frames)} symbols across {len(all_timestamps):,} 15m bars in August 2023.")
    ...
    print("\nExecuting Out-of-Sample Replay (August 2023)...")
    ...
    for t in all_timestamps:
        ...
        for sym, pos in active_positions.items():
            ...
            if bar['vwap_zscore'] >= z_target and curr_r >= 0.35:
        ...
        # 2. ENTRY EVALUATION WITH SECTOR INTERLOCK
    ...
    print("\n" + "=" * 75)
    print("ALPHA UNIVERSE: OUT-OF-SAMPLE RESULTS (AUGUST 2023)")
    print("=" * 75)
    ...
    if closed_trades:
        ...
        tdf.to_parquet('alpha_universe_august2023_oos.parquet')
        print("\nSaved detailed ledger to 'alpha_universe_august2023_oos.parquet'")
    print("=" * 75)
