...
class ZerodhaKiteLiveAdapter:
    ...
    def __init__(self, api_key: str, access_token: str):
        # ✓ FIX: Validate credentials before init (CRITICAL FIX #3)
        if not api_key or len(api_key) < 10:
            raise ValueError("❌ KITE_API_KEY missing or invalid (must be >= 10 chars)")
        if not access_token or len(access_token) < 20:
            raise ValueError("❌ KITE_ACCESS_TOKEN missing or invalid (expired? must be >= 20 chars)")
        ...
        try:
            ...
            logger.info(f"✓ Kite connection verified | Account: {profile.get('user_id')}")
        ...
        self.paper_orders = {}
        self.trade_log = []
        self._instrument_cache = {}  # Cache for tokens (optimization)
    ...
    def get_1min_candles(self, symbol: str, limit: int = 100, lookback_days: int = 2) -> List[Dict]:
        """Fetch last N 1-min candles (with reasonable lookback window)"""
        try:
            ...
            # ✓ FIX: Use dynamic date range (lookback_days, not hardcoded 2024-01-01)
            ...
            logger.info(f"✓ Fetched {len(result)} candles for {symbol} ({lookback_days}d lookback)")
            return result
        except Exception as e:
    ...
    def _get_instrument_token(self, symbol: str) -> int:
        """Get instrument token from symbol (with caching to avoid repeated downloads)"""
        ...
        try:
            ...
            if not self._instrument_cache:
                logger.info("Downloading instrument master from Kite (first time)...")
                instruments = self.kite.instruments()
                for inst in instruments:
                    if inst['segment'] == 'NSE' and inst['instrument_type'] == 'EQ':
                        self._instrument_cache[inst['tradingsymbol']] = inst['instrument_token']
                logger.info(f"✓ Cached {len(self._instrument_cache)} NSE equity symbols")

            return self._instrument_cache.get(symbol, 0)
        except Exception as e:
