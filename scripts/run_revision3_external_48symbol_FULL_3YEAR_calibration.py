#!/usr/bin/env python3
"""
Revision 3 External Engine: HMM + ANSI Protection Relay, full 3-year calibration.

Parallel to baseline external engine (without relay).
...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "output_revision3_external"
OUTPUT_DIR.mkdir(exist_ok=True)
SUMMARY_PATH = OUTPUT_DIR / "revision3_external_48symbol_FULL_3YEAR_calibration_summary.json"
CHECKPOINT_PATH = OUTPUT_DIR / "revision3_external_48symbol_FULL_3YEAR_calibration_checkpoint.json"
...
def main():
    parser = argparse.ArgumentParser(description="Revision 3 External: HMM + Protection Relay, full 3-year")
    parser.add_argument("--time-only", action="store_true", help="Run one fixed-param evaluation and exit")
    args = parser.parse_args()

    print("=" * 100, flush=True)
    print("REVISION 3 EXTERNAL ENGINE (HMM + ANSI PROTECTION RELAY): FULL 3-YEAR CALIBRATION", flush=True)
    print("=" * 100, flush=True)
    ...
    if args.time_only:
        ...
        t0 = time.time()
        orch = Revision3ExternalEngineOrchestrator(symbols, registry, starting_equity=1_000_000.0)
    ...
    run_config = CalibrationRunConfig.from_registry_defaults(registry, checkpoint_path=str(CHECKPOINT_PATH), seed=305)
    ...
    supervisor = CalibrationSupervisor(
        ...
        orchestrator_class=Revision3ExternalEngineOrchestrator,
    )
    ...
    summary = {
        "engine": "Revision3ExternalEngineOrchestrator (HMM + ANSI Relay)",
    ...
    print("\n" + "=" * 100, flush=True)
    print("REVISION 3 EXTERNAL CALIBRATION COMPLETE", flush=True)
    print("=" * 100, flush=True)
