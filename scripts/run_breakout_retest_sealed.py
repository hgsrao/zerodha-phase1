#!/usr/bin/env python3
"""Sealed 48-symbol train/validation/test for the fixed retest hypothesis."""
...
def _run(symbol,frame):
 ...
 for _,day in frame.groupby(frame.timestamp.dt.date,sort=True):
  ...
  volume_median=day.volume.shift(1).rolling(20,min_periods=20).median().to_numpy(float);vwap=(day.close*day.volume).cumsum().div(day.volume.cumsum()).to_numpy(float)
  ledger=StudyEntryShadowLedger(max_hold_bars=30);alpha=BreakoutRetestShadow(ledger)
  for i in range(60,len(day)):
   ...
   if i<len(day)-1:
    ...
    alpha.observe_precomputed(symbol,i,b.timestamp,b,scale,{'close':close[i],'prior_high':prior_high[i],'prior_low':prior_low[i],'session_vwap':vwap[i],'ema20':ema[i],'ema20_lag4':ema[i-4],'volume_ratio':volume[i]/max(volume_median[i],1e-12)})
...
def main():
 ...
 report={'run_type':'sealed_fixed_breakout_retest_shadow','manifest_hash':m.manifest_hash,'windows':windows,'alpha_contract':'20-bar breakout + original trend/volume filters; wait 1-5 bars for retest of breakout level and directional close; next-bar fill; no tuning','results':results,'path_diagnostics':{k:diagnose(all_rows[k]) for k in windows},'evaluation':evaluate_fixed_alpha(results),'note':'Shadow research only. The test window is not used to alter this hypothesis.'}
 out=Path(a.output or ROOT/'diagnostic_output'/'breakout_retest_sealed_202309_202311.json');out.write_text(json.dumps(report,indent=2,default=str));print(json.dumps({'output':str(out),'results':results,'evaluation':report['evaluation']},indent=2))
