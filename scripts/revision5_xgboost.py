...
print("=" * 80)
print("REVISION 5: XGBOOST NON-LINEAR INTERACTION REDESIGN")
print("=" * 80)
...
print("Preparing Training (Jan-Mar) and Validation (April) datasets...")
X_train, y_train = extract_dataset('train')
X_val, y_val = extract_dataset('validation')

print(f"Train Samples: {len(y_train)} (Win Rate: {y_train.mean()*100:.2f}%)")
print(f"Val Samples: {len(y_val)} (Win Rate: {y_val.mean()*100:.2f}%)")
...
clf = xgb.XGBClassifier(
    ...
    scale_pos_weight=neg_pos_ratio * 0.9, # Slightly conservative weight
    random_state=42,
    n_jobs=-1
)
...
with open('revision5/frozen_models_2026/xgboost_model.pkl', 'wb') as f:
    pickle.dump(clf, f)
