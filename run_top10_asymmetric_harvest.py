...
class TimeDecayGovernor:
    def get_time_decay_factor(self, current_time) -> float:
        ...
        return float(np.clip(remaining / total_window, 0.35, 1.0))
    ...
    def compute_scheduled_targets(self, vol_ratio: float, current_time) -> tuple[float, float]:
        ...
        base_r = 1.35 * np.clip(vol_ratio, 0.9, 1.4)
        base_z = 0.50
        
        decayed_r = max(0.65, base_r * decay) # Enforce a 0.65R minimum target floor
        decayed_z = 0.00 if decay < 0.50 else base_z * decay
...
def run():
    ...
    print("\nExecuting Asymmetric Harvest Simulation (July 2023)...")
    ...
    for t in all_timestamps:
        ...
        for sym, pos in active_positions.items():
            ...
            # Enforce >= 0.65R before admitting Z target
            if bar['vwap_zscore'] >= z_target and curr_r >= 0.65:
        ...
        for sym, df in symbol_frames.items():
            ...
            if sym in armed_symbols:
                ...
                if curr_bar['close'] > prev_bar['high'] and curr_bar['close'] > curr_bar['open']:
                    ...
                    risk_t = max(entry_p - swing_low, 1.0 * curr_bar['atr'])  # Loosen to 1.0 ATR
    ...
    print("\n" + "=" * 75)
    print("ASYMMETRIC HARVEST RESULTS (JULY 2023)")
    print("=" * 75)
    ...
    if closed_trades:
        ...
        tdf.to_parquet('top10_asymmetric_july2023.parquet')
    print("=" * 75)
