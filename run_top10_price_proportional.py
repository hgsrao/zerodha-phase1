...
TARGET_SYMBOLS = [
    ...
    "TCS", "AXISBANK", "LT", "BAJFINANCE", "TATAMOTORS"
]

SECTOR_MAP = {
    "HDFCBANK": "BANK", "ICICIBANK": "BANK", "SBIN": "BANK", "AXISBANK": "BANK",
    "INFY": "IT", "TCS": "IT",
    "RELIANCE": "ENERGY", "LT": "INFRA", "BAJFINANCE": "FIN", "TATAMOTORS": "AUTO"
}

PORTFOLIO_CAPITAL = 500000.0
MAX_CONCURRENT_POSITIONS = 4
SLOT_CAPITAL = PORTFOLIO_CAPITAL / MAX_CONCURRENT_POSITIONS  # ₹1,25,000 per slot
...
def run():
    ...
    print("Loading and precomputing features for Top 10 Heavyweights...")
    symbol_frames = {}
    ...
    print("\nExecuting Price-Proportional Risk Simulation (July 2023)...")
    ...
    for t in all_timestamps:
        ...
        # 2. ENTRY EVALUATION (PRICE-PROPORTIONAL SIZING)
        ...
        for sym, df in symbol_frames.items():
            ...
            if sym in armed_symbols:
                ...
                if curr_bar['close'] > prev_bar['high'] and curr_bar['close'] > curr_bar['open']:
                    ...
                    # Sizing is directly proportional to share price:
                    # Slot Capital / Share Price
    ...
    print("\n" + "=" * 75)
    print("PRICE-PROPORTIONAL CAPITAL REPLAY RESULTS (JULY 2023)")
    print("=" * 75)
    ...
    if closed_trades:
        ...
        for sym, g in tdf.groupby('symbol'):
            ...
            print(f"  • {sym:<10}: {len(g):<2} trades | WR: {sym_wr:5.1f}% | Avg Risk: ₹{g['risk_inr'].mean():6.1f} | Net: ₹{g['net_pnl'].sum():+9.2f}")
            
        tdf.to_parquet('top10_price_proportional_july2023.parquet')
    print("=" * 75)
