...
BASE_SLOT_CAPITAL = 333333.33  # ₹3.33L per slot
BASE_R_TARGET = 2.25
BASE_Z_TARGET = 0.60
STOP_ATR_MULT = 1.00
...
def resample_15min(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['dt'] = pd.to_datetime(raw_dt := df[t_col]).dt.tz_localize(None)
...
def replay_window(data_dir: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    for t in all_times:
        ...
        # 2. Entry Dispatcher with 50-EMA Trend Filter
...
def main():
    ...
    print("=" * 82)
    print("STARTING FUSED CALIBRATED WALK-FORWARD (R=2.25, STOP=1.0x ATR, 50-EMA FILTER)")
    print("=" * 82)
    ...
    print("\n" + "=" * 82)
    print("FUSED CALIBRATED WALK-FORWARD SUMMARY (2023 JUL - DEC)")
    print("=" * 82)
    print(f"{'Month':<18} | {'Trades':<6} | {'Win %':<6} | {'Gross P&L':<12} | {'Fees':<10} | {'Net P&L':<12}")
    print("-" * 82)
    for _, r in sdf.iterrows():
        print(f"{r['Month']:<18} | {int(r['Trades']):<6} | {r['Win_Rate']:5.1f}% | ₹{r['Gross_PNL']:+10,.2f} | ₹{r['Fees']:8,.2f} | ₹{r['Net_PNL']:+10,.2f}")
    print("-" * 82)
    print(f"{'TOTAL':<18} | {int(sdf['Trades'].sum()):<6} | {'---':<6} | ₹{sdf['Gross_PNL'].sum():+10,.2f} | ₹{sdf['Fees'].sum():8,.2f} | ₹{sdf['Net_PNL']:+10,.2f}")
    print("=" * 82)
    ...
    if all_ledgers:
        ...
        master.to_parquet("fused_calibrated_2023_ledger.parquet")
        print("\n[+] Detailed ledger saved to 'fused_calibrated_2023_ledger.parquet'")
