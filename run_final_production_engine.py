"""
Production Engine Replay: Calibrated Breadth + Organic VR Filter + Expectancy Boost
---------------------------------------------------------------------------------
Period: July 2023 - August 2026 (38 Months)
"""
...
def run_month_slice(config: EngineConfig, sym_files: dict, start_d: str, end_d: str) -> pd.DataFrame:
    ...
    for t in all_timestamps:
        ...
        # 1. Position Management with Amplified Harvest Targets
        ...
        # 2. Entries with Breadth Gate (< -0.45)
        ...
        for sym, df in symbol_frames.items():
            ...
            # Dynamic Organic Variance Ratio Check: Inhibit if VR > 1.05
...
def main():
    ...
    print("=" * 80)
    print("RUNNING FINAL CALIBRATED & EXPECTANCY-AMPLIFIED REPLAY (38 MONTHS)")
    print("=" * 80)
    ...
    for label, start_d, end_d in windows:
        ...
        print(f"{label:<16} | Trades: {tot:<2} | WR: {wr:5.1f}% | Gross: ₹{gross:+9,.2f} | Fees: ₹{fees:7,.2f} | Net: ₹{net:+9,.2f}")
    ...
    print("\n" + "=" * 80)
    print("FINAL CONSOLIDATED 38-MONTH PORTFOLIO AUDIT")
    print("=" * 80)
