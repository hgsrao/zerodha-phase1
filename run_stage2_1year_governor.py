...
def run():
    ...
    print("Running Stage 2 (1 Full Year: July 2023 - June 2024) with Sovereign Droop Governor...")
    ...
    for i, fpath in enumerate(csv_files, 1):
        ...
        print(f"[{i:02d}/48] Simulating {symbol:<12} (Droop R={droop_r*100:.1f}%)...", end='\r')
        try:
            ...
            year_df = raw[(raw['dt'] >= '2023-07-03') & (raw['dt'] <= '2024-06-28')].copy()
            if len(year_df) < 500:
                continue
    ...
    print("\n" + "=" * 75)
    print("STAGE 2 (1-YEAR) WITH SOVEREIGN DROOP GOVERNOR RESULTS")
    print("=" * 75)
    ...
    if trades:
        ...
        for r_val, grp in tdf.groupby('droop_r'):
            ...
            print(f"  • {m_type:<14}: {len(grp):<5} trades | WR: {(grp['net_r'] > 0).mean()*100:5.1f}% | Net P&L: {grp['net_r'].sum():+.2f}R")
        print("-" * 75)
        print("Exit Distribution:")
        for r, cnt in tdf['exit_reason'].value_counts().items():
            print(f"  • {r:<33}: {cnt:<5} trades ({cnt/total*100:.1f}%)")
    print("=" * 75)
