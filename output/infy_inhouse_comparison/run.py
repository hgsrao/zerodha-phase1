...
def clean(x):
    ...
    return str(x)
for case in ['infy_one_day','infy_first_seven_days']:
    source=root/'output'/case/'INFY_input.csv'
    external=json.loads((source.parent/'report.json').read_text())
    ...
    assert report['bars_processed']==external['bars_processed']
