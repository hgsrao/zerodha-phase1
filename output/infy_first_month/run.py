...
first=ts.min()
month_mask=(ts.dt.year==first.year)&(ts.dt.month==first.month)
days=sorted(ts[month_mask].dt.date.unique())
day=f'{days[0]} through {days[-1]}'
one=raw[ts.dt.date.isin(days)].copy()
...
for d in days:
    date=str(d)
    entries=[t for t in report['trades'] if t['entry_timestamp'][:10]==date]
    exits=[t for t in report['trades'] if t['exit_timestamp'][:10]==date]
    daily.append({'date':date,'bars':int((bars.timestamp.dt.date==d).sum()),'entries':len(entries),'exits':len(exits),'gross_pnl':sum(t['pnl'] for t in exits),'costs':sum(t['costs'] for t in exits),'net_pnl':sum(t['net_pnl'] for t in exits)})
assert sum(d['entries'] for d in daily)==report['fills']==len(report['trades'])
assert sum(d['exits'] for d in daily)==report['completed_trades']
assert abs(sum(d['net_pnl'] for d in daily)-report['net_pnl'])<1e-6
assert not engine.open_trades
pd.DataFrame(daily).to_csv(out/'daily_summary.csv',index=False)
