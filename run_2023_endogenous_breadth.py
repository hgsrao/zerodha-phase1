"""
2023 Batch Walk-Forward Runner with Endogenous Breadth Filter
------------------------------------------------------------
Computes synthetic market breadth across universe constituents.
Inhibits long entries during synchronized multi-asset flushes.
"""
...
class BreadthGatedEngine:
    ...
    def run_replay(self) -> pd.DataFrame:
        ...
        for t in all_timestamps:
            ...
            # Calculate composite z-score across all available assets at timestamp t
            current_z_scores = [
            ...
            # Systemic flush threshold: if average market z < -1.1, market is dumping
            mean_market_z = np.mean(current_z_scores) if current_z_scores else 0.0
            if mean_market_z < -1.1:
...
def print_terminal_pnl_chart(summary_df: pd.DataFrame):
    print("\n" + "=" * 80)
    print("ENDOGENOUS BREADTH-GATED 2023 MONTHLY NET P&L DISTRIBUTION")
    print("=" * 80)
...
def main():
    ...
    print("=" * 80)
    print("STARTING 2023 WALK-FORWARD (ENDOGENOUS BREADTH GATE)")
    print("=" * 80)

    for label, start_d, end_d in windows:
        print(f"\n[SWEEP] Processing {label} | Range: {start_d} to {end_d}...")
        engine = BreadthGatedEngine(config, DATA_DIR)
    ...
    print("\n" + "=" * 80)
    print("ENDOGENOUS BREADTH-GATED 2023 PERFORMANCE SUMMARY TABLE")
    print("=" * 80)
    ...
    if all_trade_ledgers:
        ...
        master_df.to_parquet("walkforward_2023_breadth_ledger.parquet")
        print("[INFO] Saved to 'walkforward_2023_breadth_ledger.parquet'")
