...
def compute_welford_15min_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['vol_flow_ratio'] = df['volume'] / df['vol_median'].replace(0, 1.0)
...
def run():
    ...
    print("Resampling and precomputing 15-minute features (July 2023)...")
    symbol_frames = {}
    ...
    print(f"Loaded {len(symbol_frames)} symbols across {len(all_timestamps):,} 15-minute bars.")
    ...
    active_positions = {}
    closed_trades = []
    armed_symbols = {}  # Tracks ANSI 25 Synchrocheck arming state
    
    print("Executing Option B: 15-Minute Intraday + ANSI 25 Synchrocheck Reversal Confirmation...")
    ...
    for t in all_timestamps:
        ...
        for sym, pos in active_positions.items():
            ...
            # Hard 15:15 IST intraday boundary
            if bar_time >= pd.to_datetime('15:15:00').time():
                symbols_to_close.append((sym, bar_close, "SESSION_1515_SQUAREOFF"))
                continue
                
            # Worst-case structural stop check
            if bar_low <= pos['stop_price']:
                fill = min(bar_open, pos['stop_price']) - (0.05 * risk_ticks)
                symbols_to_close.append((sym, fill, "STOP_TRIGGERED"))
                continue
                
            # VWAP Equilibrium Mean Reversion Target
            if bar['vwap_zscore'] >= -0.10:
                symbols_to_close.append((sym, bar_close, "VWAP_EQUILIBRIUM_TARGET"))
                continue
                
            # Dynamic Profit Latch
        ...
        # 2. EVALUATE CANDIDATE ENTRIES VIA ANSI 25 SYNCHROCHECK
        ...
        for sym, df in symbol_frames.items():
            ...
            # Phase 1: Arming State (Extreme Oversold Detected)
            ...
            # Phase 2: ANSI 25 Synchrocheck Reversal Confirmation
            if sym in armed_symbols:
                # Update lowest point while armed
                armed_symbols[sym]['swing_low'] = min(armed_symbols[sym]['swing_low'], curr_bar['low'])
                
                # Check for confirmed bullish breakout of prior bar high
    ...
    print("\n" + "=" * 75)
    print("OPTION B: 15-MINUTE INTRADAY + ANSI 25 SYNCHROCHECK RESULTS")
    print("=" * 75)
    ...
    if closed_trades:
        ...
        tdf.to_parquet('option_b_15min_ledger.parquet')
    print("=" * 75)
