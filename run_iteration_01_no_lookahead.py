...
def run_iteration_1():
    ...
    print(f"ITERATION 1: No Future Leakage. Processing {len(csv_files)} files...\n")
    all_trades = []
    
    for i, file_path in enumerate(csv_files, 1):
        symbol = file_path.name.split('_')[1]
        print(f"[{i}/{len(csv_files)}] Processing {symbol}...", end='\r')
        ...
        for pos in range(375, len(df) - 40): 
            ...
            # ITERATION 1 FIX: MPC lookahead removed entirely. Strict causal entry.
    ...
    print("\n" + "=" * 70)
    print("ITERATION 1: RESULTS (LOOKAHEAD REMOVED)")
    print("=" * 70)
    if all_trades:
        ...
        print(f"Net Expectancy (post-friction): {portfolio_net_exp:+.3f}R")
    else:
