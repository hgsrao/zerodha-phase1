"""L2 DATASET CERTIFICATION TOOL V2 — 2026-08-25

Corrected from V1:

1. Full-session coverage independently anchored to frozen 09:15–15:15 IST grid
   (1,440 theoretical 15-second slots).
   Separate observed-span quality metric (does NOT replace full-session check).

2. Removed arbitrary 5% depth-anomaly escalation threshold.
   Depth anomalies reported and counted; HOLD/FAIL decision driven only by
   structural corruption and symbol/chronology defects, not percentage rules.

3. Opening/closing coverage detection: reports missing first/last slots explicitly.

All other V1 behavior preserved: read-only, sparse-depth preservation,
source anomaly reporting, immutable decisions.
"""
...
# Frozen full-session grid: 09:15:00 to 15:14:45 IST
# = 21,600 seconds / 15 = 1,440 theoretical snapshot slots
FULL_SESSION_SECONDS = 21600  # 6 hours
FULL_SESSION_EXPECTED_SLOTS = int(FULL_SESSION_SECONDS / FROZEN_INTERVAL_SECONDS)  # 1,440
...
class CertificationResult:
    ...
    # V2 CHANGE: Full-session coverage independently calculated
    full_session_expected_slots: int = FULL_SESSION_EXPECTED_SLOTS
    full_session_expected_observations: int = FULL_SESSION_EXPECTED_SLOTS * EXPECTED_SYMBOL_COUNT
    observed_snapshots_in_session: int = 0

    # Separate observed-span quality (does NOT replace full-session check)
    observed_span_seconds: float = 0.0
    observed_span_expected_slots: int = 0
    opening_coverage: float = 0.0  # % of first 15 min covered
    closing_coverage: float = 0.0  # % of last 15 min covered
...
def certify_session(
    ...
    *,
    ...
    V2 improvements:
    - Full-session coverage anchored independently to 09:15–15:15 IST grid
    - Observed-span quality calculated separately
    - Opening/closing coverage detection
    - No arbitrary percentage thresholds
    """
    ...
    last_timestamp: datetime | None = None
    first_timestamp_ist: datetime | None = None
    ...
    with jsonl_path.open("r", encoding="utf-8") as f:
        for line_num, line in enumerate(f, start=1):
            ...
            try:
                ...
                result.anomalies.malformed_json.append((line_num, str(e)))
                result.status = "FAIL"
                continue
            ...
            if snapshot.timestamp_ist:
                ...
                if first_timestamp_ist is None:
                    first_timestamp_ist = snapshot.timestamp_ist
            ...
            # Check depth schema (structural issues regardless of overall validity)
            # Malformed depth container (not a list)
            depth_raw = raw.get("depth", {})
            if isinstance(depth_raw, dict):
                buy_raw = depth_raw.get("buy")
                sell_raw = depth_raw.get("sell")
                if buy_raw is not None and not isinstance(buy_raw, list):
                    result.anomalies.invalid_depth_structure.append(
                        (line_num, symbol, "BUY_NOT_LIST")
                    )
                if sell_raw is not None and not isinstance(sell_raw, list):
                    result.anomalies.invalid_depth_structure.append(
                        (line_num, symbol, "SELL_NOT_LIST")
                    )

            # Missing populated levels in valid snapshots
            if snapshot.valid:
            ...
            if snapshot.is_crossed():
                result.anomalies.crossed_books.append((line_num, symbol))
            if snapshot.is_locked():
                result.anomalies.locked_books.append((line_num, symbol))
    ...
    # V2 CHANGE: Full-session coverage independently calculated
    # Full session: 09:15:00 to 15:14:45 IST = 1,440 theoretical slots × 48 symbols
    result.full_session_expected_slots = FULL_SESSION_EXPECTED_SLOTS
    result.full_session_expected_observations = FULL_SESSION_EXPECTED_SLOTS * EXPECTED_SYMBOL_COUNT
    result.observed_snapshots_in_session = result.valid_snapshots

    # Separate observed-span quality (does NOT replace full-session check)
    if result.first_timestamp and result.last_timestamp:
        result.observed_span_seconds = (
            result.last_timestamp - result.first_timestamp
        ).total_seconds()
        result.observed_span_expected_slots = max(
            1, int(result.observed_span_seconds / FROZEN_INTERVAL_SECONDS) + 1
        )
    ...
    result.depth_statistics = {
        ...
        "invalid_depth_count": len(result.anomalies.invalid_depth_structure),
    }

    # Determine final status
    # V2 CHANGE: Removed arbitrary 5% threshold; use structural defects only
    if result.status == "FAIL":
        ...
        result.status = "FAIL"
          ...
          len(result.anomalies.invalid_depth_structure) > 0):  # ANY structural depth issue -> HOLD
        result.status = "HOLD"
    # V2 NEW: Incomplete full-session coverage -> HOLD
    elif result.observed_snapshots_in_session < 0.95 * result.full_session_expected_observations:
        result.status = "HOLD"
...
def write_certification(
    ...
    md_lines = [
        f"# L2 Dataset Certification V2 — {result.session_date or 'Unknown'}",
        "",
        ...
        "",
        "## Full-Session Coverage (Independent Grid)",
        "",
        f"- Expected (09:15–15:15 IST): {result.full_session_expected_slots} snapshot slots",
        f"- Expected symbol-observations: {result.full_session_expected_observations:,}",
        f"- Observed valid snapshots: {result.observed_snapshots_in_session:,}",
        f"- Coverage: {100 * result.observed_snapshots_in_session / result.full_session_expected_observations:.1f}%",
        "",
        "## Observed-Span Quality (Separate Metric)",
        "",
        f"- Observed span: {result.observed_span_seconds:.0f} seconds",
        f"- Expected slots in observed span: {result.observed_span_expected_slots}",
        "",
        "## Findings",
        "",
        ...
        f"- Invalid depth structures: {result.depth_statistics.get('invalid_depth_count', 0)}",
    ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python l2_dataset_certifier_v2.py <path/to/raw_l2_snapshots.jsonl>"
        )
        sys.exit(1)
