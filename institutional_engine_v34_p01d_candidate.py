...
# NSE-notified full trading holidays for calendar year 2026 (equity segment).
# Cross-checked against two independent published NSE 2026 holiday calendars
# on 2026-08-14. Saturdays/Sundays are excluded by the weekday check below and
# do not need to be listed here. Muhurat trading (Sun 2026-11-08) is a special
# extra session, not a holiday, and is intentionally omitted.
...
def describe_nse_trading_window(
    ...
    *,
    ...
    """Return (is_open, reason). `now` must be an Asia/Kolkata-aware datetime.

    This is a defense-in-depth gate on *new* entries only. It never inspects
    or blocks management/exit of an already active trade - a position must
    always be resolvable regardless of the clock.
    """
...
class Config:
    ...
    # NOT currently wired into the production risk gate's effective daily-
    # loss limit (see run_production_p01d_candidate.py's RunnerEntryAuthorizer
    # construction and BRAIN_RESEARCH_SPEC_V15_RISK_CONFIG_REALIGNMENT.md).
    # The real, operative daily-loss protection is daily_hard_halt_pct below.
    # Kept for backward compatibility with existing callers/tests rather than
    # removed outright.
    ...
    # Per-trade and daily/weekly/drawdown risk fractions below were checked
    # 2026-08-14 against current retail/prop-firm risk-management standards
    # (TradeZella, PropFirmShop, MyFundedCapital risk-rule surveys): 0.5-1%
    # risk per trade, 3-5% daily loss limit, 5-15% max drawdown for a
    # personal trial account. These sit inside or on the conservative edge
    # of those ranges and were left unchanged.
    target_risk_pct: Decimal = Decimal("0.005")        # 0.5% per trade
    max_trade_risk_pct: Decimal = Decimal("0.0075")    # 0.75% per trade cap
    daily_entry_lock_pct: Decimal = Decimal("0.015")   # 1.5% - stop new entries
    daily_hard_halt_pct: Decimal = Decimal("0.02")     # 2% - real daily-loss halt
    rolling_week_halt_pct: Decimal = Decimal("0.04")   # 4%
    trial_drawdown_halt_pct: Decimal = Decimal("0.05") # 5% - conservative end of 5-15%
    daily_turnover_pct: Decimal = Decimal("1.00")
    max_daily_entries: int = 2
    # NOT currently enforced by count: the production risk gate hardcodes
    # "any existing exposure blocks a new entry" regardless of this value
    # (run_production_p01d_candidate.py's SIMULTANEOUS_POSITION_LIMIT check).
    # See BRAIN_RESEARCH_SPEC_V15_RISK_CONFIG_REALIGNMENT.md - making this
    # field real is scoped as future work, not done here.
    ...
    # NSE regular equity session window, Asia/Kolkata local time. New entries
    # are refused outside this window and on configured holidays; an already
    # active trade is never blocked from being managed or exited by this gate.
...
class TradeContext:
    ...
    # Immutable durable entry-submission intent. Once ENTRY_SUBMITTING is
    # persisted, recovery may observe/adopt the broker order but never submit
    # a second BUY automatically.
...
class TradingEngineV34:
    ...
    def request_entry(
        ...
        *,
        ...
        is_open, window_reason = describe_nse_trading_window(
            ...
            market_open_time=self.cfg.market_open_time,
            market_close_time=self.cfg.market_close_time,
        )
        if not is_open:
    ...
    def _active_positions(positions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Return nonzero MIS exposure; ignore only explicit CNC holdings."""
        active, _ignored_cnc = classify_positions(positions)
        return active
    ...
    def _build_entry_submission_fingerprint(
        ...
        return {
            ...
            "order_type": "LIMIT",
            "quantity": int(quantity),
            "price": self._canonical_decimal_string(price),
            "tag": "V3.4_ENTRY",
        }

    @staticmethod
    def _order_matches_entry_fingerprint(
        order: Dict[str, Any], fingerprint: Dict[str, Any]
    ) -> bool:
        if not isinstance(order, dict) or not isinstance(fingerprint, dict):
            return False
        for field in (
            "exchange", "tradingsymbol", "transaction_type", "product",
            "order_type", "tag",
        ):
            if order.get(field) != fingerprint.get(field):
                return False
        try:
    ...
    def _reconcile_unknown_entry(self) -> str:
        ctx = self.state.active_trade
        if not ctx or not ctx.entry_submission_fingerprint:
            self.trigger_hard_halt(
                "P0-1D: ENTRY_UNKNOWN/ENTRY_SUBMITTING lacks immutable submission fingerprint."
            )
            return "HALTED"

        try:
            orders = self._observe(
                "get_orders:entry_reconciliation", self.broker.get_orders
            )
            ...
            required = {
                ...
                "product", "order_type", "quantity", "price", "tag", "status",
            }
            for order in orders:
                if not isinstance(order, dict) or required.difference(order):
                    raise RuntimeError(
                        "FAIL_CLOSED: Broker order is missing required entry reconciliation fields."
                    )

            matches = [
                order for order in orders
                if self._order_matches_entry_fingerprint(
                    order, ctx.entry_submission_fingerprint
                )
            ]
            if len(matches) > 1:
                self.trigger_hard_halt(
                    "P0-1D: Multiple exact V3.4 entry orders match the immutable fingerprint.",
                    source="ENTRY_RECONCILIATION",
                )
                return "HALTED"
            if len(matches) == 1:
                ...
                self.audit.log(
                    ...
                    source_state="ENTRY_UNKNOWN",
                )
                return "STATE_CHANGED"

            ctx.entry_reconciliation_failures = int(
                getattr(ctx, "entry_reconciliation_failures", 0) or 0
            ) + 1
            if self.state.status == "ENTRY_SUBMITTING":
                self.state.status = "ENTRY_UNKNOWN"
            self.store.save(self.state)
            if ctx.entry_reconciliation_failures > int(self.cfg.observation_retry_budget):
                self.trigger_hard_halt(
                    "P0-1D: Entry reconciliation observation budget exhausted without an exact broker-order match.",
                    source="ENTRY_RECONCILIATION",
                )
                return "HALTED"
            return "NO_ACTION"
            ...
            if "FAIL_CLOSED" in str(exc):
                self.trigger_hard_halt(
                    f"P0-1D: Malformed entry reconciliation observation: {exc}",
                    source="ENTRY_RECONCILIATION",
                )
                return "HALTED"
            ctx.entry_reconciliation_failures = int(
            ...
            if ctx.entry_reconciliation_failures > int(self.cfg.observation_retry_budget):
                self.trigger_hard_halt(
                    f"P0-1D: Entry reconciliation failed repeatedly: {exc}",
                    source="ENTRY_RECONCILIATION",
                )
                return "HALTED"
            return "NO_ACTION"
    ...
    def reconcile_startup(self) -> None:
        ...
        try:
            ...
            # Dispatch reconciliation from the durable pre-restart state.  The
            # transient RECONCILING marker must not erase the recovery seam.
            ...
            # ENTRY_SUBMIT is a durable, pre-side-effect boundary.  With no
            # recorded order id it is safe to resume only when the broker is
            # completely clean; any broker activity is ambiguous and halts.
            if persisted_status == "ENTRY_SUBMIT" and local_trade.entry_order_id is None:
                if active_pos or active_orders:
                    self.trigger_hard_halt(
                        "Reconciliation Failure: Persisted ENTRY_SUBMIT has ambiguous broker activity."
                    )
                    return
                if not local_trade.symbol:
                    self.trigger_hard_halt(
                        "Reconciliation Failure: Local active trade is missing symbol."
                    )
                    return
                self.state.status = "ENTRY_SUBMIT"
                self.store.save(self.state)
            ...
            # A persisted entry side-effect boundary is ambiguous by design.
            # Recovery is observation-only and must never issue another BUY.
            if persisted_status in {"ENTRY_SUBMITTING", "ENTRY_UNKNOWN"}:
                self.state.status = persisted_status
                self._reconcile_unknown_entry()
                return
    ...
    def step(self) -> str:
        ...
        try:
            if self.state.status == "STARTUP":
                ...
                return "NO_ACTION"
            ...
            elif self.state.status in {"ENTRY_SUBMITTING", "ENTRY_UNKNOWN"}:
                    return self._reconcile_unknown_entry()
                    ...
                    # No existing order matches. Persist the immutable intent and
                    # side-effect boundary before making the only permitted BUY call.
                    fingerprint = self._build_entry_submission_fingerprint(
                        symbol=ctx.symbol,
                        quantity=ctx.target_qty,
                    ...
                    if ctx.entry_submission_fingerprint is not None:
                        if ctx.entry_submission_fingerprint != fingerprint:
                            self.trigger_hard_halt(
                                "P0-1D: Existing entry fingerprint conflicts with current intent.",
                                source="ENTRY_SUBMIT",
                            )
                            return "HALTED"
                    else:
                    ...
                    ctx.entry_reconciliation_failures = 0
                    self.state.status = "ENTRY_SUBMITTING"
                    self.store.save(self.state)
                    ...
                    try:
                        ...
                        if self._is_transient_submission_exception(exc):
                            ...
                            return "STATE_CHANGED"
                        self.trigger_hard_halt(
                            f"CRITICAL P0: Entry submission rejected/failed with known outcome: {exc}",
                            source="ENTRY_SUBMIT",
                        )
                        return "HALTED"

                    if not isinstance(order_id, str) or not order_id.strip():
                        # The broker call returned, but without a usable identity.
                        # Acceptance cannot be disproved, so this is an ambiguous
                        # outcome and must enter observation-only reconciliation.
                        self.state.status = "ENTRY_UNKNOWN"
                        self.store.save(self.state)
                        return "STATE_CHANGED"
