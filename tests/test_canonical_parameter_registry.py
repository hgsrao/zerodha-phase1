...
def test_revision_2_manifest_surface_contract_is_exact():
    ...
    assert len(expected_names) == 136
    assert len(set(expected_names)) == 136

    registry = CanonicalParameterRegistry()

    assert set(expected_names) == set(registry.params)
    assert registry.total_target_surface() == 136
    ...
    # Engine-scoped: 109 across engines = in-house 47 + external-only 62 (see surface_counts()).
...
def test_registry_identity_is_frozen_and_matches_contract():
    ...
    assert registry.FROZEN_IDENTITY_SHA256 == (
        "e103019e83e384f7ddc9b410abdc860d3753386ba0d3450524d517519f0fc2ed"
    )
...
def test_registry_black_box_mapping_and_fixed_surface_are_consistent():
    ...
    # 28 fixed (22 base + 5 external cost/regularization/shadow controls + 1 diagnostic); 108 eligible across engines.
    assert len(registry.fixed_target_names()) == 28
