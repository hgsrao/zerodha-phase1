...
def test_revision_2_manifest_surface_contract_is_exact():
    ...
    assert len(set(expected_names)) == 69

    registry = CanonicalParameterRegistry()

    assert set(expected_names) == set(registry.params)
    assert registry.total_target_surface() == 69
    # 47, not 45: rebalance_frequency_minutes (FIXED, non-calibratable --
    ...
    assert len(registry.calibratable_names()) == 47
...
def test_registry_black_box_mapping_and_fixed_surface_are_consistent():
    ...
    # 22/47, not 23/45 -- see test_revision_2_manifest_surface_contract_is_exact's comment.
    assert len(registry.fixed_target_names()) == 22
    assert len(set(registry.calibratable_names())) == 47
