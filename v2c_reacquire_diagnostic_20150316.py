"""V2-C targeted reacquisition DIAGNOSTIC for 2015-03-16. Read-only.
...
Purpose: 2015-03-16 shows an extremely precise pattern - exactly the
09:15 bar-open slot missing, uniformly, across all 47 affected files
(see V2C_CALENDAR_DATABASE_CORRECTION_20260818.md "Round 4"). A targeted
web-search pass already ran and found no documented NSE incident on
this date - this diagnostic is the higher-value next experiment,
because a fresh pull can distinguish two very different explanations
that external search cannot:
  A. Persistent upstream Kite gap: fresh pull ALSO misses 09:15 while
     later bars are present - analogous in evidentiary structure to
     2016-01-01/2015-01-16, but slot-level rather than full-day.
  B. Original acquisition defect / transient API issue: fresh pull
     RECOVERS 09:15 - a candidate repair operation, NOT auto-applied;
...
starting 2015-02-02, well before this target date - unlike the
2015-01-16 diagnostic, this window is NOT inside their edge-truncation
block) plus NIFTY 50.

Narrow window: 2015-03-12..2015-03-18 (a few real trading days on each
side of the target date, a Monday).

Report, per instrument, exactly the fields requested:
  fresh 09:15 present? / original 09:15 present? /
...
TARGET_DATE = "2015-03-16"
TARGET_SLOT = "09:15:00"
WINDOW_START = date(2015, 3, 12)
WINDOW_END = date(2015, 3, 18)
...
# 5 real trading days (Thu, Fri, Mon, Tue, Wed) x 25 slots/session.
EXPECTED_SLOTS = 5 * 25
...
def main() -> int:
    ...
    for sym in SAMPLE_SYMBOLS + [CONTROL_SYMBOL]:
        ...
        row_report = {
            ...
            "fresh_09:15_present": fresh_present,
            "original_09:15_present": orig_present,
        ...
        print(f"    fresh 09:15 present?      {fresh_present}")
        print(f"    original 09:15 present?   {orig_present}  ({orig_file})")
    ...
    print("\n=== Summary table ===")
    print(f"{'symbol':12s} {'fresh_0915':11s} {'orig_0915':10s} {'exp/ret':9s} {'dupes':6s}")
    for r in report_rows:
        print(f"{r['symbol']:12s} {str(r['fresh_09:15_present']):11s} {str(r['original_09:15_present']):10s} "
              f"{EXPECTED_SLOTS}/{r['fresh_returned_slots']:<6d} {r['duplicates']}")

    equities_missing_fresh = [r for r in report_rows if r["symbol"] != CONTROL_SYMBOL and r["fresh_09:15_present"] is False]
    equities_recovered_fresh = [r for r in report_rows if r["symbol"] != CONTROL_SYMBOL and r["fresh_09:15_present"] is True]
    ...
    if len(equities_missing_fresh) == len(SAMPLE_SYMBOLS):
        print("ALL sampled equities STILL missing 09:15 in the fresh pull ->")
        print("  strong evidence of a persistent upstream Kite slot-level defect (analogous")
        print("  in evidentiary structure to 2016-01-01/2015-01-16, but slot-level). Check the")
        print("  NIFTY 50 result above too before concluding.")
    elif len(equities_recovered_fresh) == len(SAMPLE_SYMBOLS):
        print("ALL sampled equities RECOVERED 09:15 in the fresh pull ->")
