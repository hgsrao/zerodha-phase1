...
class BlackBox10_CalibratedGovernor:
    ...
    def evaluate_entry(self, raw_pid_u: float, nifty_vel: float) -> tuple[bool, float]:
        ...
        return (u_eff >= 0.65), u_eff

    def evaluate_in_trade(self, current_r: float, peak_r: float, z_score: float) -> tuple[bool, str]:
        # Tier 1: Cost-Clearing Latch (Peak >= +0.65R)
        # Lock stop to +0.25R (guaranteeing net positive after 0.18R friction)
        if peak_r >= 0.65 and current_r <= 0.25:
            return True, "GOVERNOR_COST_PROTECT_EXIT"

        # Tier 2: Major Gain Lock (Peak >= +1.2R)
        # Trail at 65% of peak
        if peak_r >= 1.20 and current_r <= peak_r * 0.65:
            return True, "GOVERNOR_RUNAWAY_PROFIT_HARVEST"

        # Target: Full equilibrium reached
        if z_score >= -0.20:
...
class GainScheduledPID:
    ...
    def update(self, current_z: float, vol_r: float) -> float:
        ...
        self.integral = np.clip(self.integral + error, -10.0, 10.0)
...
def run():
    ...
    print("Running Calibrated Hysteresis Governor on Stage 1 (July 2023)...")
    
    for i, fpath in enumerate(csv_files, 1):
        symbol = fpath.name.split('_')[1]
        droop_r = MACHINE_DROOP_MAP.get(symbol, DEFAULT_DROOP)
        bb10 = BlackBox10_CalibratedGovernor(droop_r=droop_r)
    ...
    print("\n" + "=" * 75)
    print("STAGE 1 WITH CALIBRATED HYSTERESIS GOVERNOR RESULTS")
    print("=" * 75)
