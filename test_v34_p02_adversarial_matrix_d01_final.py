...
def test_P02_02_malformed_orders_payload_fails_closed_immediately():
    cases = [
        ...
        Broker(orders=[dict(MATCH)]),
    ]

    # Case 1: top-level response is not a list.
    # Case 2: an order record is missing a required reconciliation field.
    cases[1].orders_data[0].pop("market_protection")
