...
def compute_causal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Anchored Intraday VWAP (Resetting daily per timestamp)
    ...
    # Expanding session standard deviation for Z-Score
...
def run_month_dryrun():
    ...
    print(f"=== STAGE 1: 1-MONTH CAUSAL DRY RUN (July 2023) ===")
    all_trades = []
    
    for i, file_path in enumerate(csv_files, 1):
        symbol = file_path.name.split('_')[1]
        print(f"[{i:02d}/48] Loading {symbol}...", end='\r')
        ...
        try:
            ...
            # Restrict strictly to first month: 2023-07-03 to 2023-08-04
        ...
        for pos in range(0, len(df) - 45):
            ...
            # Causal entry evaluation at close of bar pos
    ...
    print("\n" + "=" * 70)
    print("STAGE 1 (1-MONTH) DRY RUN PERFORMANCE SUMMARY")
    print("=" * 70)
    if all_trades:
        ...
        print(f"Total Trades (1 Month): {total_t}")
        ...
        print("No trades triggered during this 1-month test window.")
    print("=" * 70)
